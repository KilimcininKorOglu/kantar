...
export default function Users() {
  ...
  const handleDelete = async (id: number) => {
    ...
    catch { setError(t('users.failedToDelete')) }
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { setShowCreate(true); setError('') }}
          ...
          <Plus className="w-3.5 h-3.5" /> {t('users.createUser')}
        </button>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border text-text-dim">
              <th className="text-left px-4 py-2.5 text-xs font-medium">{t('users.username')}</th>
              <th className="text-left px-4 py-2.5 text-xs font-medium">{t('common.email')}</th>
              <th className="text-left px-4 py-2.5 text-xs font-medium">{t('common.role')}</th>
          ...
          <tbody>
            {users.length === 0 ? (
              <tr><td colSpan={5} className="px-4 py-10 text-center text-text-dim text-xs">{t('users.noUsersFound')}</td></tr>
            ) : users.map((u) => (
...
function RoleBadge({ role }: { role: string }) {
  const cls = role === 'super_admin' ? 'text-accent bg-accent/10' : role === 'registry_admin' ? 'text-blue-300 bg-blue-400/10' : 'text-text-muted bg-surface-2'
  ...
  return (
    <div className="bg-surface border border-border rounded p-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-xs font-semibold text-text">{t('users.createUser')}</h3>
...
function EditUserForm({ user, onClose, onUpdated }: { user: User; onClose: () => void; onUpdated: () => void }) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault(); setLoading(true); setError('')
    try { await api.put(`/users/${user.id}`, { email, role, active }); onUpdated() }
    catch { setError(t('users.failedToUpdate')) }
    setLoading(false)
  }
  ...
  return (
    <div className="bg-surface border border-border rounded p-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-xs font-semibold text-text">{t('users.editUser', { username: user.username })}</h3>
        <button onClick={onClose} className="text-text-dim hover:text-text cursor-pointer"><X className="w-4 h-4" /></button>
      </div>
      {error && <div className="bg-danger/10 border border-danger/20 text-danger text-xs rounded px-3 py-2 mb-3">{error}</div>}
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
        ...
        <label className="flex items-center gap-2 text-xs text-text-muted">
          <input type="checkbox" checked={active} onChange={(e) => setActive(e.target.checked)} className="rounded" /> {t('common.active')}
        </label>
        <button type="submit" disabled={loading} className="bg-accent hover:bg-accent-hover disabled:opacity-50 text-white text-xs font-medium rounded py-1.5 cursor-pointer">{loading ? t('common.saving') : t('common.save')}</button>
      </form>
    </div>
  )
}
