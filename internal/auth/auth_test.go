...
func TestMatchNamespace(t *testing.T) {
	tests := []struct {
		...
		{"@types/*", "@other/pkg", false},
	}

	for _, tt := range tests {
		result := MatchNamespace(tt.pattern, tt.name)
		if result != tt.expected {
			t.Errorf("MatchNamespace(%q, %q) = %v, want %v", tt.pattern, tt.name, result, tt.expected)
		}
	}
}
...
func TestGenerateAPIToken(t *testing.T) {
	...
	if prefix != plaintext[:12] {
		t.Error("prefix should match first 12 chars of plaintext")
	}
